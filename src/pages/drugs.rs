                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div class="form-group" style="grid-column: 1 / -1;">
                                    ...
                                    <div class="flex gap-2 mb-2">
                                        <select onchange={let default_usage = default_usage.clone(); Callback::from(move |e: Event| {
                                            let val = e.target_unchecked_into::<HtmlInputElement>().value();
                                            if !val.is_empty() && val != "เลือกจากรายการที่ใช้บ่อย..." {
                                                default_usage.set(val);
                                            }
                                        })} style="flex: 1;">
                                            <option value="">{ "📋 เลือกจากรายการที่ใช้บ่อย..." }</option>
                                            <option value="รับประทานครั้งละ 1 เม็ด วันละ 3 ครั้ง หลังอาหาร">{ "รับประทานครั้งละ 1 เม็ด วันละ 3 ครั้ง หลังอาหาร" }</option>
                                            <option value="รับประทานครั้งละ 2 เม็ด วันละ 3 ครั้ง หลังอาหาร">{ "รับประทานครั้งละ 2 เม็ด วันละ 3 ครั้ง หลังอาหาร" }</option>
                                            <option value="รับประทานครั้งละ 1 เม็ด วันละ 2 ครั้ง หลังอาหาร เช้า-เย็น">{ "รับประทานครั้งละ 1 เม็ด วันละ 2 ครั้ง เช้า-เย็น" }</option>
                                            <option value="รับประทานครั้งละ 1 เม็ด ก่อนอาหาร 30 นาที">{ "รับประทานครั้งละ 1 เม็ด ก่อนอาหาร 30 นาที" }</option>
                                            <option value="รับประทานครั้งละ 1 เม็ด ก่อนนอน">{ "รับประทานครั้งละ 1 เม็ด ก่อนนอน" }</option>
                                            <option value="รับประทานครั้งละ 1 ช้อนชา วันละ 3 ครั้ง หลังอาหาร">{ "รับประทานครั้งละ 1 ช้อนชา วันละ 3 ครั้ง หลังอาหาร" }</option>
                                            <option value="รับประทานครั้งละ 1.5 ช้อนชา วันละ 3 ครั้ง หลังอาหาร">{ "รับประทานครั้งละ 1.5 ช้อนชา วันละ 3 ครั้ง หลังอาหาร" }</option>
                                            <option value="รับประทานครั้งละ 2 ช้อนชา วันละ 3 ครั้ง หลังอาหาร">{ "รับประทานครั้งละ 2 ช้อนชา วันละ 3 ครั้ง หลังอาหาร" }</option>
                                            <option value="รับประทานครั้งละ 2.5 cc วันละ 3 ครั้ง หลังอาหาร">{ "รับประทานครั้งละ 2.5 cc วันละ 3 ครั้ง หลังอาหาร" }</option>
                                            <option value="รับประทานครั้งละ 5 cc วันละ 3 ครั้ง หลังอาหาร">{ "รับประทานครั้งละ 5 cc วันละ 3 ครั้ง หลังอาหาร" }</option>
                                            <option value="รับประทานเมื่อมีอาการ">{ "รับประทานเมื่อมีอาการ" }</option>
                                            <option value="ทาบริเวณที่เจ็บ วันละ 2-3 ครั้ง">{ "ทาบริเวณที่เจ็บ วันละ 2-3 ครั้ง" }</option>
                                            <option value="หยอดตา ครั้งละ 1-2 หยด วันละ 3-4 ครั้ง">{ "หยอดตา ครั้งละ 1-2 หยด วันละ 3-4 ครั้ง" }</option>
                                            <option value="หยอดหู ครั้งละ 3-5 หยด วันละ 2-3 ครั้ง">{ "หยอดหู ครั้งละ 3-5 หยด วันละ 2-3 ครั้ง" }</option>
                                            <option value="เขย่าขวดก่อนรับประทาน">{ "เขย่าขวดก่อนรับประทาน" }</option>
                                        </select>
                                    </div>
                                    <input type="text" value={(*default_usage).clone()}
                                        placeholder="หรือพิมพ์เอง เช่น รับประทานครั้งละ 1 เม็ด วันละ 3 ครั้ง หลังอาหาร"
                                ...
                                <div class="form-group" style="grid-column: 1 / -1;">
                                    ...
                                    <div class="flex gap-2 mb-2">
                                        <select onchange={let warning = warning.clone(); Callback::from(move |e: Event| {
                                            let val = e.target_unchecked_into::<HtmlInputElement>().value();
                                            if !val.is_empty() && val != "เลือกจากรายการที่ใช้บ่อย..." {
                                                warning.set(val);
                                            }
                                        })} style="flex: 1;">
                                            <option value="">{ "⚠️ เลือกคำเตือนที่ใช้บ่อย..." }</option>
                                            <option value="ห้ามดื่มแอลกอฮอล์">{ "ห้ามดื่มแอลกอฮอล์" }</option>
                                            <option value="ห้ามขับรถหรือทำงานกับเครื่องจักร">{ "ห้ามขับรถหรือทำงานกับเครื่องจักร" }</option>
                                            <option value="ทำให้ง่วงซึม">{ "ทำให้ง่วงซึม" }</option>
                                            <option value="ห้ามกินพร้อมนม/ยาลดกรด">{ "ห้ามกินพร้อมนม/ยาลดกรด" }</option>
                                            <option value="ห้ามใช้ในหญิงตั้งครรภ์">{ "ห้ามใช้ในหญิงตั้งครรภ์" }</option>
                                            <option value="ห้ามหยุดยาเอง">{ "ห้ามหยุดยาเอง" }</option>
                                            <option value="กินยาจนหมด">{ "กินยาจนหมด" }</option>
                                            <option value="เก็บในที่เย็น">{ "เก็บในที่เย็น" }</option>
                                            <option value="อาจทำให้ปัสสาวะเป็นสีส้ม">{ "อาจทำให้ปัสสาวะเป็นสีส้ม" }</option>
                                            <option value="ระวังในผู้ป่วยโรคไต">{ "ระวังในผู้ป่วยโรคไต" }</option>
                                            <option value="ระวังในผู้ป่วยโรคตับ">{ "ระวังในผู้ป่วยโรคตับ" }</option>
                                        </select>
                                    </div>
                                    <input type="text" value={(*warning).clone()}
                                        placeholder="หรือพิมพ์เอง เช่น ห้ามดื่มแอลกอฮอล์, ทำให้ง่วงซึม"
